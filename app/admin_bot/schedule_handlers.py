...
from app.admin_bot.handlers import (
    ...
    ScheduleSettingsWeekdaysConfirm, ScheduleSettingsInterval, AdminMenuCB, can_access_safe_feature
)
...
async def schedule_settings(callback: types.CallbackQuery, state: FSMContext) -> None:
    if not can_access_safe_feature(callback.from_user.id):
...
Введите дату начала цикла (формат DD-MM-YYYY):
...
📅 Дата: {date_str}
✓ День теперь доступен для бронирования""",
            parse_mode="HTML",
            reply_markup=schedule_mode_keyboard()
        )
        await state.clear()
...
Возможно, есть записи на выходные дни. Удалите их или выберите другую дату.""",
            parse_mode="HTML",
            reply_markup=schedule_mode_keyboard()
        )
...
Возможно, есть записи на выходные дни.""",
            parse_mode="HTML",
            reply_markup=schedule_mode_keyboard()
        )
...
Выходные дни можно блокировать кнопкой '📆 График'""",
        ...
        await query.message.edit_text(
            "❌ Ошибка при установке свободного режима",
            reply_markup=schedule_mode_keyboard()
        )
...
@schedule_router.callback_query(ScheduleOpenDay.filter())
